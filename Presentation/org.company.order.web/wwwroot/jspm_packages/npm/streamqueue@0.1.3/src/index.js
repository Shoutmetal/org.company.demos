...
function StreamQueue(options) {
  ...
  this._queueState._internalStream = new Stream.Writable(isStream(options) || 'function' === typeof options ? undefined : options);
  ...
  Stream.Readable.call(this, isStream(options) || 'function' === typeof options ? undefined : options);
  if (arguments.length > 1 || isStream(options) || 'function' === typeof options) {
    this.done.apply(this, [].slice.call(arguments, isStream(options) || 'function' === typeof options ? 0 : 1));
  }
}
StreamQueue.prototype.queue = function() {
  ...
  streams = streams.map(function(stream) {
    function wrapper(stream) {
      ...
      if ('undefined' == typeof stream._readableState) {
        stream = (new Stream.Readable({objectMode: _self._queueState._objectMode})).wrap(stream);
      }
  ...
  this._queueState._streams = this._queueState._streams.length ? this._queueState._streams.concat(streams) : streams;
...
Object.defineProperty(StreamQueue.prototype, 'length', {get: function() {
