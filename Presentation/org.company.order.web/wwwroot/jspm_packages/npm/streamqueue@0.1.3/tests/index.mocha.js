...
var StreamQueue = require('../src/index');
      ...
      describe('in binary mode', function() {
        describe('and with async streams', function() {
          it('should work with functionnal API', function(done) {
            StreamQueue(StreamTest[version].fromChunks(['wa', 'dup']), StreamTest[version].fromChunks(['pl', 'op']), StreamTest[version].fromChunks(['ki', 'koo', 'lol'])).pipe(StreamTest[version].toText(function(err, text) {
              if (err) {
                done(err);
              }
          ...
          it('should fire end asynchronously with streams', function(done) {
            var queue = new StreamQueue();
            var ended = false;
            queue.queue(StreamTest[version].fromChunks(['wa', 'dup']).on('end', function() {
              assert.equal(ended, false);
            }));
            queue.queue(StreamTest[version].fromChunks(['pl', 'op']).on('end', function() {
              assert.equal(ended, false);
            }));
            queue.queue(StreamTest[version].fromChunks(['ki', 'koo', 'lol']).on('end', function() {
              assert.equal(ended, false);
            }));
            assert.equal(queue.length, 3);
            queue.pipe(StreamTest[version].toText(function(err, text) {
        ...
        describe('and with sync streams', function() {
          it('should work with functionnal API', function(done) {
            ...
            StreamQueue(stream1, stream2, stream3).pipe(StreamTest[version].toText(function(err, text) {
              if (err) {
                done(err);
              }
              assert.equal(text, 'wadupplopkikoolol');
        ...
        describe('and with functions returning streams', function() {
          it('should work with functionnal API', function(done) {
            StreamQueue(StreamTest[version].fromChunks.bind(null, ['wa', 'dup']), StreamTest[version].fromChunks.bind(null, ['pl', 'op']), StreamTest[version].fromChunks.bind(null, ['ki', 'koo', 'lol'])).pipe(StreamTest[version].toText(function(err, text) {
              if (err) {
                done(err);
              }
              assert.equal(text, 'wadupplopkikoolol');
